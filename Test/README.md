...
# 결과: 2024-09-29 ~ 2024-10-02 날짜별로 학생식당과 교직원식당이 모두 포함된 JSON 파일 생성
```

### 출력 파일 예시
```
output/
├── Inha_University_2024-09-29.json  # 9월 29일 (학생식당 + 교직원식당)
├── Inha_University_2024-09-30.json  # 9월 30일 (학생식당 + 교직원식당)
├── Inha_University_2024-10-01.json  # 10월 1일 (학생식당 + 교직원식당)
└── Inha_University_2024-10-02.json  # 10월 2일 (학생식당 + 교직원식당)
```

## 주의사항

1. **Gemini API 키 필요**: `config.py`에 API 키가 설정되어 있어야 합니다.
2. **웹사이트 이용약관 준수**: 각 웹사이트의 이용약관을 확인하고 준수하세요.
3. **동적 콘텐츠 대응**: JavaScript가 많이 사용된 사이트는 `--wait` 옵션으로 대기 시간을 늘려보세요.
4. **로그 확인**: 실행 과정은 `scraper.log` 파일에 상세히 기록됩니다.
5. **파일 병합**: 같은 날짜의 파일이 이미 있으면 새로운 식당 정보가 자동으로 추가됩니다.

## 문제 해결

- **Playwright 오류**: `python3 -m playwright install chromium` 명령어를 실행하세요.
- **Gemini API 오류**: `config.py`의 API 키가 올바른지 확인하세요.
- **스크래핑 실패**: `--wait 5000` 옵션으로 대기 시간을 늘려보세요.
- **JSON 형식 오류**: Gemini API 모델이 최신 버전인지 확인하세요.
- **메모리 부족**: 너무 많은 URL을 한 번에 처리하지 말고 나누어 실행하세요.
